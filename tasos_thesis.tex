...
We then focused on the number of tRNA isoacceptors per amino acid, and \gls{isodecoders} per anticodon. We noticed a wide range of pre-tRNA counts per isoacceptor (\textbf{Fig. \ref{paper4}B}), with our data providing read evidence for 47 out of 62 coding codons (61 canonical and 1 selenocysteine TAG). 
...
Instead, we observed no strong correlation between precursor and mature tRNA read counts in either of the two techniques (R < 0.2; \textbf{\ref{supp4}}). 
...
Correlation of relative read frequencies (log2-transformed) between pre-tRNA (x-axis) and mature tRNAs (y-axis) for SSB PAR-CLIP (A) and hydro-tRNAseq (B). Correlation was calculated using the Pearson test. Linear fit is shown in red. The y=x line (dotted) is shown for comparison.
}
\centering
\label{supp4}%
\end{figure}
